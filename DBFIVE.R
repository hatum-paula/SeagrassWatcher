...
#' $s = list of state-labels (as indices) for each node in $n$
#' @export
#' @examples none
#' @name filtercpt
...
#' and updJcpt to calculate marginals for X(t) for all t, compute slice partial results for $\pi(t)$,
...
# --------------------------------------------------------------------------------------------------
#' @title initupdateI (NEW!)
#' @description Update I and evidence for Pi based inferencing 
#' @param dbn - old DBFIVE I object
#' @param maxdt - max dt-slice (=2 for Markov system)
#' @return list containing:
#' \itemize{
#'  \item $nnames (char vec of node names),
...
#' @examples none
#' @name initupateI
initupdateI = function(dbn,tend,maxdt) {
  # ----------------------- Conversion from DBNSMILER to DFIVE -----------------------
  nnames = dbn$nnames
  statenames = dbn$statenames
  parnodeind = dbn$parnodeind
  parnodetslice = dbn$parnodetslice
  childnodeind = dbn$childnodeind
  childnodetslice = dbn$childnodetslice
  numstates = dbn$numstates
  cpt = dbn$cpt
